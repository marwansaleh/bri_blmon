...
function loadReportByWilayah()
{
    ...
    //get all bidang
    $sql = "SELECT id, type FROM program_types ORDER BY sort asc";
    ...
    if ($kanwils)foreach($kanwils as $kanwil){
        ...
        foreach($bidangs as $bidang){
            ...
            if ($result)
            {
                ...
                $arr_bidang_detail = array("budget"=>number_format($budget,2,',','.'),"benef_orang"=>number_format($benef_orang,0,',','.'),"benef_unit"=>number_format($benef_unit,0,',','.'));
        ...
        $arr_data['bidang']['kanwil'][$i]['tot_budget'] = number_format($tot_budget,2,",","."); 
        ...
        if ($i>0&&$i%$recs_perpage==0)
        {
            ...
            {
                $item['budget'] = number_format($item['budget'],2,",",".");
